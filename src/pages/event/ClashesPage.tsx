...
import { Shield, AlertTriangle, CheckCircle2, Mail, Loader2, RefreshCw } from 'lucide-react'
...
export default function ClashesPage() {
  ...
  const unsub = onSnapshot(collection(db, 'events', eventId, 'submissions'), snap => {
    ...
    docs.forEach(d => {
      const title: string = getClashField(d)?.trim() || ''
      if (!title) return
      const key = title.toLowerCase()
      if (!groups[key]) groups[key] = { title, entries: [] }
  ...
  function getClashField(d: any): string {
    if (eventType === 'choir' || eventType === 'worship') return d.songSearch || ''
    if (eventType === 'conference') return d.topic || ''
    if (eventType === 'drama') return d.dramaTitle || ''
    return d.actTitle || d.topic || d.songSearch || ''
  }
...
>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ background: clashes.length > 0 ? 'rgba(248,113,113,0.06)' : 'rgba(19,26,46,0.7)', border: `1px solid ${clashes.length > 0 ? 'rgba(248,113,113,0.2)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 12, padding: '14px 16px' }}>
          <div style={{ fontSize: 24, fontWeight: 800, color: clashes.length > 0 ? '#F87171' : '#22C55E', fontFamily: 'var(--font-display)', marginBottom: 2 }}>{clashes.length}</div>
      ...
      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'rgba(255,255,255,0.4)', padding: '40px 0' }}>
          <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> Scanning submissions...
        </div>
      ) : clashes.length === 0 ? (
        <div style={{ ...cardStyle, padding: '60px 24px', textAlign: 'center' }}>
          <CheckCircle2 size={40} color="#22C55E" style={{ marginBottom: 16 }} />
          <h3 style={{ fontSize: 18, fontWeight: 700, color: '#fff', fontFamily: 'var(--font-display)', marginBottom: 8 }}>No clashes detected</h3>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)' }}>
            All {clashLabel}s are unique. {totalSubs > 0 ? `Checked ${totalSubs} submissions.` : 'No submissions yet.'}
          </p>
        </div>
      ) : (
          ...
          {clashes.map((clash, i) => (
            <div key={i} style={{ ...cardStyle, border: '1px solid rgba(248,113,113,0.2)', background: 'rgba(248,113,113,0.03)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 20px', borderBottom: '1px solid rgba(248,113,113,0.1)' }}>
                ...
                <span style={{ marginLeft: 'auto', fontSize: 11, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', color: '#F87171', padding: '3px 10px', borderRadius: 6, fontWeight: 600 }}>
                  CLASH
                </span>
              </div>

              {/* Entries side by side */}
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(clash.entries.length, 2)}, 1fr)`, gap: 12, padding: '16px 20px' }} className="clash-entries">
                {clash.entries.map((entry, j) => (
                  <div key={j} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      {entry.email && (
                        <a
                          href={`mailto:${entry.email}?subject=Clash Notice: ${clash.title}&body=Hi, we noticed another group has also chosen "${clash.title}". Please get in touch to resolve this clash.`}
                          style={{
      ...
      <style>{`
        ...
        @media (max-width: 600px) { .clash-entries { grid-template-columns: 1fr !important; } }
      `}</style>
    </DashboardLayout>
  )
}
